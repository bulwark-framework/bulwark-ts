...
**Invariants under composition.** Pin-once is enforced by types (`reassess` and every later block take the pinned rubric, not a reference) and verified by `assertAssessmentInvariants(history)` in the testing entry, which fails a run that scheduled `resolveRubric` twice or `decide` with a rubric hash other than the one `resolveRubric` returned. Uncertainty-reaches-a-person is enforced by the resolver, which never returns an automatic route with an uncertain answer. The bounded evidence loop is a helper the reference workflow shows; a developer who omits it has chosen to.
